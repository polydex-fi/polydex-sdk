...
import { ChainId, WMATIC as _WMATIC, TradeType, Rounding, Token, TokenAmount, Pair, Route, Trade } from '../src'
...
const WMATIC = _WMATIC[ChainId.TESTNET]
    ...
    describe(`decimals permutation: ${decimals}`, () => {
      ...
      it('Pair', () => {
        pairs = [
          ...
          new Pair(
            new TokenAmount(tokens[2], decimalize(1, tokens[2].decimals)),
            new TokenAmount(WMATIC, decimalize(1234, WMATIC.decimals))
          )
        ]
      })
      ...
      it('Route', () => {
        ...
        expect(route.path).toEqual(tokens.concat([WMATIC]))
        expect(route.input).toEqual(tokens[0])
        expect(route.output).toEqual(WMATIC)
      })
      ...
      describe('Trade', () => {
          ...
          route = new Route(
            [
              new Pair(
                ...
                new TokenAmount(WMATIC, decimalize(10, WMATIC.decimals))
              )
            ],
            tokens[1]
          )
          const inputAmount = new TokenAmount(tokens[1], decimalize(1, tokens[1].decimals))
          const expectedOutputAmount = new TokenAmount(WMATIC, '1662497915624478906')
        ...
        it('TradeType.EXACT_OUTPUT', () => {
          const outputAmount = new TokenAmount(WMATIC, '1662497915624478906')
      ...
      it('TokenAmount', () => {
        const amount = new TokenAmount(WMATIC, '1234567000000000000000')
